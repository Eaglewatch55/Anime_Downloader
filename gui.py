...
def scan_button ():
    ...
    path = cb_save.get()
    update_log(f"Carpeta de Descarga: {path}")
    ...
    for show in shows:
        ...
        if script.new_episode(ep_url):
            ...
            episode_name= f"{show}_{str(next_episode)}.mp4"

            if url_zippy != False:
                mp4_link = script.z_scrap(url_zippy)

                if mp4_link != False:
                    update_log(f"Descargando {episode_name}")
                    
                    if script.download(mp4_link,episode_name, path):
                        update_log("Descarga finalizada")
                        query.update_chapter(show)
                    else:
...
cb_save = ttk.Combobox(tab_scan,state="readonly", values=["//192.168.100.1/usb-058f-CE8B2A_1/Video/Descargas","D:/Temp/CAPITULOS DESCARGA"])
